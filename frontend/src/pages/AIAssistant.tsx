...
export default function AIAssistant() {
  ...
  const send = async (text: string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        role: 'assistant',
        content: 'Произошла ошибка. Проверьте подключение к серверу и настройку ANTHROPIC_API_KEY.',
      }])
    } finally {
      setLoading(false)
    }
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      {/* Sidebar */}
      <aside className="w-64 bg-white border-r border-gray-100 flex flex-col flex-shrink-0 p-4">
